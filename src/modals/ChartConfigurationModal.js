...
const ChartConfigurationModal = ({ isOpen, onRequestClose, onConfigureChart, dataset }) => {
  ...
  const addSeries = () => {
    setChartConfig(prev => ({
      ...prev,
      series: [...prev.series, { label: '', dimension: '', yAxis: 'y-axis-0' }],
    }));
  };
  ...
  const handleConfigureChart = () => {
    const config = {
      ...chartConfig,
      labels: selectedDimensions[chartConfig.xAxisDimension] || [],
      datasets: chartConfig.series.map(s => ({
        label: s.label,
  ...
  return (
    <Modal
      ...
      className="max-w-5xl mx-auto mt-12 bg-white rounded-xl shadow-xl p-0 overflow-hidden"
    ...
    >
      <div className="flex flex-col h-full">
        ...
        <div className="bg-gray-50 p-6 border-b border-gray-200 flex justify-between items-center">
          <h2 className="text-2xl font-semibold text-gray-800">Configure Chart</h2>
          <button 
                  ...
                  >
                    ...
                    <ChevronDown 
                      ...
                      className={`text-gray-500 transition-transform duration-200 ${
                        openDimension === chartConfig.xAxisDimension ? 'transform rotate-180' : ''
                      }`} 
                    />
                  </button>
                    ...
                    <div className="p-4">
                      <div className="flex justify-between items-center mb-3">
                        <span className="text-sm text-gray-500">
                          {selectedDimensions[chartConfig.xAxisDimension].length} of {
                            Object.keys(dataset.dimension[chartConfig.xAxisDimension].category.label).length
                          } selected
                        </span>
                        <button 
                          className="text-sm text-blue-600 hover:text-blue-800"
                          onClick={() => {
                            const allValues = Object.keys(dataset.dimension[chartConfig.xAxisDimension].category.label);
                        ...
                        >
                          {selectedDimensions[chartConfig.xAxisDimension].length === 
                           Object.keys(dataset.dimension[chartConfig.xAxisDimension].category.label).length 
                            ? "Deselect All" : "Select All"}
                        </button>
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2 max-h-56 overflow-y-auto">
                        {Object.entries(dataset.dimension[chartConfig.xAxisDimension].category.label).map(([code, label]) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <span className="text-gray-700 text-sm ml-3 truncate">{label}</span>
                            </button>
                          );
                        })}
                      </div>
              ...
              {chartConfig.series.length === 0 ? (
                ...
                <div className="space-y-4">
                  {chartConfig.series.map((series, index) => (
                    <div 
                      key={index} 
                      className="flex flex-col md:flex-row md:items-center gap-4 p-4 bg-gray-50 rounded-lg border border-gray-200 hover:border-gray-300 transition-colors"
                    >
                    ...
                    <div className="relative inline-block w-12 mr-3 align-middle select-none">
                      ...
                      <label 
                        ...
                        className={`block overflow-hidden h-6 rounded-full cursor-pointer transition-colors duration-200 ease-in-out ${
                          chartConfig.dualAxis ? 'bg-blue-600' : 'bg-gray-300'
                        }`}
                      >
                        <span 
                          className={`block h-6 w-6 rounded-full bg-white shadow transform transition-transform duration-200 ease-in-out ${
                            chartConfig.dualAxis ? 'translate-x-6' : 'translate-x-0'
                          }`}
                        ></span>
                      </label>
                    </div>
                  ...
                  <div className="flex items-center">
                    <div className="relative inline-block w-12 mr-3 align-middle select-none">
                      ...
                      <label 
                        ...
                        className={`block overflow-hidden h-6 rounded-full cursor-pointer transition-colors duration-200 ease-in-out ${
                          chartConfig.stacked ? 'bg-blue-600' : 'bg-gray-300'
                        }`}
                      >
                        <span 
                          className={`block h-6 w-6 rounded-full bg-white shadow transform transition-transform duration-200 ease-in-out ${
                            chartConfig.stacked ? 'translate-x-6' : 'translate-x-0'
                          }`}
                        ></span>
                      </label>
                    </div>
                  ...
                  <div className="flex items-center">
                    <div className="relative inline-block w-12 mr-3 align-middle select-none">
                      ...
                      <label 
                        ...
                        className={`block overflow-hidden h-6 rounded-full cursor-pointer transition-colors duration-200 ease-in-out ${
                          chartConfig.autoScale ? 'bg-blue-600' : 'bg-gray-300'
                        }`}
                      >
                        <span 
                          className={`block h-6 w-6 rounded-full bg-white shadow transform transition-transform duration-200 ease-in-out ${
                            chartConfig.autoScale ? 'translate-x-6' : 'translate-x-0'
                          }`}
                        ></span>
                      </label>
                    </div>
